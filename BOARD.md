...
| U2 | R2 — rebuild the balance day by day on the tariff, fixed charges on the month's first recharge, VAT, balance line with recharge markers | Rimjhim | `u2-tariff-engine` | pushed | 19:05 — `u2-tariff-engine` pushed at `6c30d00`. tariff.js implements SPEC's full export shape; 37 tests pass under `node --test src/lib/tariff.test.mjs` (52 with U1's); `npm run build` and preflight pass. Verified locally at desktop and 390px: line over all 181 days, marker at each of the 18 recharges, and selecting 1 Apr shows the counter reset 0 → 13 with the rate back to ৳4.63. Awaiting merge + deploy for the live check. |
